...
function finalizeRound(code) {
  ...
  for (const [pid, pdata] of room.players.entries()) {
    ...
    const avgReaction = times.length
  ...
  io.to(code).emit('room_update', {
    code,
    level: Math.min(room.level, room.maxLevel),
  ...
  setTimeout(() => {
    ...
    const seq = Array.from(
      { length: 2 + effectiveLevel },
      () => Math.floor(Math.random() * room.gridSize)
    )
    ...
    io.to(code).emit('phase_sequence', {
...
io.on('connection', (socket) => {
  ...
  socket.on('target_hit', ({ code }) => {
    ...
    const reactionMs = room.targetSpawnTime
      ? Date.now() - room.targetSpawnTime
      : 0
